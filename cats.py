...
def about(subject):
    ...
    def select(paragraph):
        paragraph_check = (split(lower(remove_punctuation(paragraph))))
        return any(word in paragraph_check for word in subject)
    return select
    # END PROBLEM 2
...
def accuracy(typed, source):
    ...
    if not typed_words or not source_words:
        return 0.0
    num = min(len(typed_words), len(source_words))
    matches = 0
    for i in range(num):
...
def wpm(typed, elapsed):
    ...
    time_passed = elapsed / 60
    return num_of_words / time_passed
    # END PROBLEM 4
...
def furry_fixes(typed, source, limit):
    ...
    """
    # BEGIN PROBLEM 6
    def helper(i, j, current_diff):
        if current_diff > limit:
            return current_diff
        if i == len(typed) or j == len(source):
            return current_diff + abs(len(typed) - i - (len(source) - j))
        if typed[i] == source [j]:
            return helper(i + 1, j + 1, current_diff)
        return helper(i + 1, j + 1, current_diff + 1)
    return helper(0, 0, 0)
    # END PROBLEM 6
...
def final_diff(typed, source, limit):
    ...
    def helper(i, j, current_diff):
        ...
        if typed[i] == source[j]:
            return helper(i + 1, j + 1, current_diff)
        add = 1 + helper(i, j + 1, current_diff + 1)
        remove = 1 + helper(i + 1, j, current_diff + 1)
        substitute = 1 + helper(i + 1, j + 1, current_diff + 1)
        return min(add, remove, substitute)
    return helper(0, 0, 0)
...
def time_per_word(words, timestamps_per_player):
    ...
    for player_times in tpp:
        ...
        for i in range(1, len(player_times)):
            ...
            player_word_times.append(duration)
...
def fastest_words(words_and_times):
    ...
    for word_index in word_indices:
        ...
        for player in player_indices:
            time = get_time(times, player, word_index)
            if time < fastest_time:
                fastest_time = time
                fastest_player = player
    
        fastest_words_by_player[fastest_player].append(words[word_index])
